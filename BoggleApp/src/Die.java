...
public class Die {
	...
	private static final int FACE_VALUE_NOT_SET = -1;
	private int faceValue = FACE_VALUE_NOT_SET;
	...
	public Die(String[] faces) {
		...
		if (faces.length == 0) {
			throw new IllegalArgumentException("faces cannot be empty");
		}
	...
	public Die(Die other) {
		...
		this(other.valueMap.values().toArray(new String[other.valueMap.size() - 1]));
	...
	public String roll() {
		...
		Random r = new Random();
		int randomValue = 1 + r.nextInt(this.getNumberOfFaces());
		this.faceValue = randomValue;

		return this.valueMap.get(randomValue);
	}
	...
	public String getValue() {
		...
		if (this.faceValue == Die.FACE_VALUE_NOT_SET) {

			return this.valueMap.get(this.valueMap.size());
		}

		return this.valueMap.get(faceValue);
	}
	...
	public SortedMap<Integer, String> getValueMap() {
		...
		SortedMap<Integer, String> fieldMapCopy = new TreeMap<Integer, String>();
		// Deep copy below
		for (int i = 0; i < this.valueMap.size(); i++) {
			fieldMapCopy.put(i + 1, this.valueMap.get(i + 1));
		}
		// System.out.println(fieldMapCopy);
		return fieldMapCopy;
	}
	...
	public int hashCode() {
		...
		int code = 0;
		for (int i = 0; i < this.valueMap.size(); i++) {
			code += this.valueMap.get(i + 1).hashCode();
		}
	...
	public boolean equals(Object obj) {
		...
		if (obj.getClass() != Die.class) {

			return false;
		} else {
			Die castedObj = (Die) obj;

			if (castedObj.getNumberOfFaces() != this.getNumberOfFaces()) {

				return false;
			} else if (!(castedObj.getValue().equals((String) this.getValue()))) {

				return false;
			} else if (castedObj.getValue().equals((String) this.getValue())) {
				List<String> tempThis = new ArrayList<String>(
						Arrays.asList(this.valueMap.values().toArray(new String[this.valueMap.size() - 1])));

				List<String> tempObj = new ArrayList<String>(
						Arrays.asList(castedObj.valueMap.values().toArray(new String[castedObj.valueMap.size() - 1])));

				java.util.Collections.sort(tempThis);
				java.util.Collections.sort(tempObj);
				if (!tempThis.equals(tempObj)) {
	...
	public String toString() {
		...
		String stringified = "";
		for (int i = 0; i < this.valueMap.size(); i++) {
			stringified += this.valueMap.get(i + 1);
			stringified += ",";
			stringified += " ";
		}
